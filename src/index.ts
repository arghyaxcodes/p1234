...
import { Font, type FontEditor, woff2 } from 'fonteditor-core';
import ora from 'ora';
import { template } from './template';
import type { GoogleFont, GoogleFontsApiResponse } from './types';
import { T } from './utils';

dotenv.config();

const code_dist = path.resolve('generated');
...
async function build(force_rebuild = false) {
    ...
    const google_font_list = await fetch(
        ...
        .then((json: GoogleFontsApiResponse) => json.items as GoogleFont[]);
